...
def trainModel(train_images, k=100):
    print("SIFT - PROCESS")
    ...
    print("KMEANS")
    if checkKmeans(k, "bovw/results/KMEANS"):
        print("\t LOADING KMEANS")
        all_visual_words,KmeansModel = importKmeans(k,'bovw/results/KMEANS')
        print("\t Done")
    else:
        _,all_visual_words, KmeansModel = kmeans(k, descriptor_list)
        print("\t EXPORT KMEANS : " + image)
        exportKMEANS(all_visual_words, KmeansModel, k, 'bovw/results/KMEANS')
        print("\t Done \n")
